...
public class RelativeLayoutTemplate extends LayoutTemplate {
	...
	public RelativeLayoutTemplate(String id, String name) {
		super(id, name);
	}
	@Override
	public ViewGroup initLayout(Context context) {
		...
		if(null != this.width && null != this.height){
			layout.setLayoutParams(new RelativeLayout.LayoutParams(this.getWidth(), this.getHeight()));
		}
	...
	public int getBgImg(Context context){
		try{
	...
	public Integer getWidth() {
		...
		try{
			if("FILL_PARENT".equalsIgnoreCase(width)){
				return RelativeLayout.LayoutParams.FILL_PARENT;
			...
			Log.i("dynamic", "width that in RelativeLayout must be Integer.");
		}
		return -1;
	}
	...
	public Integer getHeight() {
		...
		try{
			...
			Log.i("dynamic", "height that in RelativeLayout must be Integer.");
		}
