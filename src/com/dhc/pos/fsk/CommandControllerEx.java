...
public class CommandControllerEx extends CommandController {
	...
	public CommandReturn Get_CheckMAC(int MAC_mode, int keyindex,
			String Random, String data) {
		return super.Get_CheckMAC(MAC_mode, keyindex, null, data.getBytes());
	}
	...
	public CommandReturn Get_ConOperator(int mode, String Parameter_Random,
			String cash, String append_data, int timer) {
		return super.Get_ConOperator(mode, Parameter_Random.getBytes(), cash.getBytes(), append_data.getBytes(), timer);
	}

	public CommandReturn Get_EncTrack(int DES_mode, int keyindex,
			String Random, int timer) {
		return super.Get_EncTrack(DES_mode, keyindex, null, timer);
	}

	public CommandReturn Get_ExtConOperator(int mode, String Parameter_Random,
			String cash, String append_data, int timer) {
		return super.Get_ExtConOperator(mode, Parameter_Random.getBytes(), cash.getBytes(), append_data.getBytes(),
				timer);
	}

	public CommandReturn Get_ExtCtrlConOperator(int mode, int PINkeyindex,
			int DESkeyindex, int MACkeyindex, String Ctrl_mode,
			String Parameter_Random, String cash, String append_data, int timer) {
		return super.Get_ExtCtrlConOperator(mode, PINkeyindex, DESkeyindex,
				MACkeyindex, HexToBin(Ctrl_mode)[0], HexToBin(Parameter_Random), HexToBin(cash), HexToBin(append_data), timer);
	}
	...
	public CommandReturn Get_PIN(int PIN_mode, int keyindex, String cash,
			String Random, String PAN, int timer) {
	...
	public CommandReturn Set_PtrData(int current_package, int total_package,
		...
		return super.Set_PtrData(current_package, total_package, prt_count, HexToBin(prt_data), timer);
	}
	***/
	
	/**
	public CommandReturn Set_PtrData(){
		...
		return super.Set_PtrData(2, test, 60);
	}
	***/
	...
	public CommandReturn Set_PtrData(String str){
		try{
			...
			String[] content = new String[]{
					 "11    商户名称："+AppDataCenter.getValue("__MERCHERNAME")+"\n商户存根            请妥善保管\n", //第一份的标题
			         "12    商户名称："+AppDataCenter.getValue("__MERCHERNAME")+"\n持卡人存根          请妥善保管\n", //第二份的标题
			         "00商户编号（MERCHAANT NO） " + fieldsMap.get("field42"),
			         "00终端编号（TERMINAL NO） " + fieldsMap.get("field41"),
			         "00发卡行 (BANK NAME)：" + fieldsMap.get("issuerBank"),
			         "00卡号（CARD NO）" ,
			         "00    " + StringUtil.formatAccountNo(fieldsMap.get("field2")),
			         "00卡有效期 (VALID DATE) " + fieldsMap.get("field15"),
			         "00交易类型（TRANS TYPE）",
			         "00          " + AppDataCenter.getTransferName(fieldsMap.get("fieldTrancode")),
			         "00交易时间（DATE/TIME）",
			         "00    " + DateUtil.formatDateTime(fieldsMap.get("field13")+fieldsMap.get("field12")),
			         "00交易金额（RMB）",
			         "00    " + StringUtil.String2SymbolAmount(fieldsMap.get("field4")),
			         "00参考号（REFERENCE NO）" + fieldsMap.get("field37"),
			         "00批次号 (BATCH NO) " + fieldsMap.get("field60").substring(2, 8),
			         //"00授权号 (AUTH NO) " + fieldsMap.get("field38"),
			         "00流水号 (SERIAL NO) " + fieldsMap.get("field11"),
			         "00备注（REFERENCE）:",
