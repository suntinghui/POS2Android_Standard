...
public class cnValue<T> {
	...
	public cnValue(cnType t, T value) {
		...
		if ((this.datatype == cnType.LLVAR) || (this.datatype == cnType.LLLVAR)) {
		...
		} else if ((this.datatype == cnType.LLNVAR)
				|| (this.datatype == cnType.LLLNVAR)) {
			...
			if ((t == cnType.LLNVAR) && (this.length > 99))
				throw new IllegalArgumentException(
						"LLNVAR can only hold values up to 99 chars");

			if ((t == cnType.LLLNVAR) && (this.length > 999))
				throw new IllegalArgumentException(
						"LLLNVAR can only hold values up to 999 chars");
		} else {
	...
	public void write(OutputStream outs, boolean binary, Object fieldId)
			throws IOException {
		
		if (binary) {
			byte[] buf = (byte[]) null;
			
			if (this.datatype == cnType.NUMERIC || this.datatype == cnType.LLNVAR
					|| this.datatype == cnType.LLLNVAR) {
				buf = new byte[this.length / 2 + this.length % 2];
			} else if (this.datatype == cnType.AMOUNT) {
				buf = new byte[6];
			} else if ((this.datatype == cnType.DATE10)
					|| (this.datatype == cnType.DATE4)
					|| (this.datatype == cnType.DATE_EXP)
					|| (this.datatype == cnType.TIME)) {
	...
	public void writeBinary(OutputStream outs, boolean binary, Object fieldId)
			throws IOException {
