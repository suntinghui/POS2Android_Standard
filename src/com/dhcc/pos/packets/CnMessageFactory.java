...
//			ConfigParser.createFromXMLConfigFile(null);
	...
	/**
	 * 字段出现的消息中的顺序，格式 ：( msgtypeid, fieldID)
	 * 	 msgtypeid 消息类型
	 * 	 fieldID   字段id
	 */
	 ...
	 * 系统跟踪号
	 */
	public static cnSystemTraceNumGenerator SystraceNumGen;
	...
	/**
	 * 通过字节数组来创建消息
	 * @param buf 8583 消息字节数组
	 * @param msgTPDUlength TPDU长度
	 * @param msgheaderlength 报文长度
	 * @return
	 */
	public CnMessage parseMessage(byte[] respMsg,int msgTPDUlength, int msgHeaderLength)
		...
		for(int fieldnum = 2; fieldnum <= 128; fieldnum++){
			if(bs.get(fieldnum )) {	
				if(! index.contains(Integer.valueOf(fieldnum))) { // 不包含时
					System.out.println("收到类型为["  + m.getMsgTypeID() 
		...
		for (Integer i : index) {
			...
			if (bs.get(i)) {
				...
				if(i==62){
					...
					System.out.println("\tField=【" + i + "】\t< " + m.getField(i).getType() + "  >\t^" + pos + "^\t(" + m.getField(i).getLength() + ")\t["  
